...
<!-- Hoja de estilos embebida en HTML -->
<style type="text/css">
table{
	display:block;
	margin-left:40%;
...
<?php
	...
	{		
		...
		echo "\t\t<a href='".sprintf($gstrFileName,$gintImages+1)."'>\n";
		//Escribe la etiqueta de la imagen. (Se utiliza sprintf para actualizar el valor del nombre del fichero)
		echo "\t\t\t<img  src='".sprintf($gstrFileName,$gintImages+1)."' alt='".str_replace(".jpg","",sprintf($gstrFileName,$gintImages+1))."' width='200px' height='200px'/>\n";
		echo "\t\t</a>\n";
		echo "\t\t<div id='caption'>\n";
		echo "\t\t\t<a href='".sprintf($gstrFileName,$gintImages+1)."'>";
