...
//FUNCIONES
	...
	*/
	...
	public function getFormPlace($pintDestino)
	{
		...
		$fstrForm = $fstrForm."\t<form action=\"".$this->saveContentForm("Destinos",($pintDestino != -999),"Nombre","IDLugar",$pintDestino)."\" method=\"POST\" class=\"form-signin\" role=\"form\">\n";
	 ...
	 */	
	private function saveContentForm($pstrTable,$pbolUpdate,$pstrField,$pstrIdField,$pintIdValue)
	{
		if($pbolUpdate)
		{
	...
	private function createTablePlaces()
	{
		...
		for($i=0;$i< $this->resultado->num_rows; $i++)
		{
			...
			foreach($fields as $column)
			{
				$fstrTable = $fstrTable."\t\t\t\t<td><a href=\"http://192.168.1.13/html/DAW/PROYECTO05/Index.php?id=5&amp;iddestino=".$fila[0]."\">".$fila[$fintIndex]."</a></td>\n";
				$fintIndex += 1;
			}
			$fintIndex = 0;
			$fstrTable = $fstrTable."\t\t\t</tr>";			
		...
		$fstrTable = $fstrTable."\t\t\t\t<tr>\n";
		$fstrTable = $fstrTable."\t\t\t\t\t<th><a href=\"http://192.168.1.13/html/DAW/PROYECTO05/Index.php?id=6\">Añadir nuevo lugar</a></th>\n";
		$fstrTable = $fstrTable."\t\t\t\t</tr>\n";
		$fstrTable = $fstrTable."\t\t\t</tfoot>\n";
		$fstrTable = $fstrTable."</table>";
